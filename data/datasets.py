...
#         self.rain_path=os.path.join(self.data_path, 'synthetic')
#         self.norain_path=os.path.join(self.data_path, 'gt')
#         self.train = train
#
#     def __getitem__(self, index):
#         if self.train:
#             rain_file = "rain-%d.png" % (index + 1)
#             norain_file='norain-%d.png' % (index + 1)
#         else:
#             rain_file = "rain-%03d.png" % (index + 1)
#             norain_file='norain-%03d.png' % (index + 1)
#
...
#         return len(imgs)
class Rain100H_Dataset(data.Dataset):
    def __init__(self, data_path='', train=True, size=240):
        super(Rain100H_Dataset, self).__init__()
        self.data_path = data_path
        self.rain_path = os.path.join(self.data_path, 'synthetic')
        self.norain_path = os.path.join(self.data_path, 'gt')
        self.train = train
        self.size = size  # 要裁剪的目标尺寸
    ...
    def __getitem__(self, index):
        ...
        rain_img = cv2.merge([r, g, b])
        rain_img = np.float32(normalize(rain_img))
        rain_img = rain_img.transpose(2, 0, 1)  # 转换为 (C, H, W)
        ...
        norain_img = cv2.merge([r, g, b])
        norain_img = np.float32(normalize(norain_img))
        norain_img = norain_img.transpose(2, 0, 1)  # 转换为 (C, H, W)
        ...
        if self.train:
            # 使用 RandomCrop 来裁剪图像
            i, j, h, w = tfs.RandomCrop.get_params(rain_img, output_size=(self.size, self.size))
            rain_img = FF.crop(rain_img, i, j, h, w)
            norain_img = FF.crop(norain_img, i, j, h, w)
...
def get_trainloader(args):
    ...
    ots_traindata = RESIDE_Dataset(path + '/train/RESIDE', train=True, size=args.crop_size, format='.jpg')
    rain100h_traindata = Rain100H_Dataset(path + '/train/Rain100H', train=True, size=args.crop_size)
    snow100k_traindata = Snow100K_Dataset(path + '/train/Snow100K', train=True, size=args.crop_size)
...
def get_testloader(args):
    ...
    ots_testdata = RESIDE_Dataset(path + '/test/RESIDE', train=False, size='whole img', format='.png')
    rain100h_testdata = Rain100H_Dataset(data_path=path + '/test/Rain100H', train=False, size='whole img')
    snow100k_testdata = Snow100K_Dataset(path + '/test/Snow100K', train=False, size='whole img')
...
def get_loader(args):
    ...
    ots_loadertrain = DataLoader(dataset=ots_traindata, batch_size=1, shuffle=False)
    rain100h_loadertrain = DataLoader(dataset=rain100h_traindata, batch_size=1, shuffle=False)
    snow100k_loadertrain = DataLoader(dataset=snow100k_traindata, batch_size=1, shuffle=False)
